...
resultFileName  =  ['S', int2str(data.individual.id) , '_CI_b2' ];
...
DrawFormattedText(window, 'Welcome to the experiment! You are about to start Block 2.',...
    'center', 'center', white);
...
for trial = 1:length(schedule.struct)
    ...
    % if order = 2, target - first sti - un-rating
    ...
    elseif ind < 2/3
        data.output.s2_order(trial,1) = 2;
